...
@section('content')
    <section class="overflow-hidden bg-white dark:bg-adh-blue border border-adh-border dark:border-adh-blue rounded p-4 sm:p-5">
        ...
        <div class="grid grid-cols-1 gap-3 min-[430px]:grid-cols-2 sm:grid-cols-3 sm:gap-4 md:grid-cols-4">
            @foreach ($allCities as $citySlug => $cityLabel)
                ...
                <a href="{{ \App\Support\LocalizedUrl::route('city.show', ['slug' => $citySlug]) }}"
                   class="group relative flex min-w-0 flex-col items-center justify-center rounded-lg border border-adh-border p-4 transition-all duration-200 hover:border-adh-red hover:shadow-md dark:border-gray-700 sm:p-5
