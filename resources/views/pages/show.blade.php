...
@section('content')
    @php
        ...
        $trustPages = [
            ['label' => __('Hakkımızda'), 'slug' => 'hakkimizda', 'url' => \App\Support\LocalizedUrl::route('page.about')],
            ['label' => __('Gizlilik Politikası'), 'slug' => 'gizlilik-politikasi', 'url' => \App\Support\LocalizedUrl::route('page.privacy')],
            ['label' => __('KVKK Aydınlatma Metni'), 'slug' => 'kvkk', 'url' => \App\Support\LocalizedUrl::route('page.kvkk')],
            ['label' => __('Çerez Politikası'), 'slug' => 'cerez-politikasi', 'url' => \App\Support\LocalizedUrl::route('page.cookies')],
            ['label' => __('İletişim'), 'url' => \App\Support\LocalizedUrl::route('contact')],
        ];
    @endphp

    <section class="space-y-5 md:space-y-6">
        <div class="rounded-[var(--adh-radius)] border border-adh-border bg-white px-5 py-6 shadow-[var(--adh-shadow)] dark:border-gray-700 dark:bg-adh-blue md:px-8 md:py-9">
            <p class="text-[10px] font-semibold uppercase tracking-[0.24em] text-adh-red md:text-[11px]">{{ $pageContext['eyebrow'] }}</p>
            <h1 class="mt-3 max-w-3xl font-serif text-[1.85rem] font-bold leading-tight text-adh-text dark:text-gray-100 md:text-4xl">
                {{ $page->title }}
            </h1>
        ...
        <div class="grid grid-cols-1 gap-5 xl:grid-cols-12 xl:gap-6">
            <aside class="xl:col-span-3">
                <div class="rounded-[var(--adh-radius)] border border-adh-border bg-slate-50/80 p-4 dark:border-gray-700 dark:bg-adh-blue md:p-5">
            ...
            <article class="rounded-[var(--adh-radius)] border border-adh-border bg-white p-5 shadow-[var(--adh-shadow)] dark:border-gray-700 dark:bg-adh-blue md:p-8 xl:col-span-9">
                <div class="prose prose-neutral max-w-none dark:prose-invert prose-headings:font-serif prose-headings:tracking-tight prose-h2:text-[1.45rem] prose-h2:mt-8 prose-h2:mb-3 prose-h3:text-xl prose-h3:mt-6 prose-p:leading-8 prose-p:text-[1.02rem] prose-p:text-adh-text dark:prose-p:text-gray-200 prose-ul:my-4 prose-li:my-1 prose-a:text-adh-red hover:prose-a:text-red-700">
                    {!! $page->content !!}
                </div>
            </article>
        </div>
